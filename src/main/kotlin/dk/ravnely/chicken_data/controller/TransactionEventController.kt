...
@Inject
constructor(val transactionEventService: TransactionEventService) {
  ...
  fun createTransactionEvent(transactionEventInput: TransactionEventInput): TransactionEventOutput {
    return transactionEventService
        .createTransactionEvent(transactionEventInput.toInternal())
        .let(TransactionEventOutput::fromInternal)
  }
  ...
  fun putTransactionEvent(
    ...
    return transactionEventService
        .updateTransactionEvent(id, transactionEventInput.toInternal())
        ?.let(TransactionEventOutput::fromInternal)
        ?.let { Response.ok(it).build() }
        ?: Response.status(Response.Status.NOT_FOUND).build()
  }
  ...
  @Path("{id}")
  @Consumes(MediaType.APPLICATION_JSON)
  fun deleteTransactionEvent(@PathParam("id") id: Long): Response {
